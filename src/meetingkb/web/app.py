...
def opensearch_query_body(query: str, meeting_id: str | None, term: str | None, limit: int) -> dict:
    ...
    if query.strip():
        ...
        sort = [{"_score": {"order": "desc"}}, {"meeting_date": {"order": "asc"}}, {"start_sec": {"order": "asc"}}]
    else:
...
def run_search(query: str, meeting_id: str | None, term: str | None, limit: int, available: bool) -> list[SearchHit]:
    try:
        ...
        return search_sqlite(query, meeting_id, term, limit)
    except (requests.RequestException, OpenSearchError) as exc:
        st.warning(f"OpenSearch is unavailable, falling back to SQLite FTS. {exc}")
        return search_sqlite(query, meeting_id, term, limit)
...
def main() -> None:
    ...
    with st.spinner("Searching transcripts…"):
        hits = run_search(query, meeting_id, selected_term, int(limit), available)
